...
//  * waypoint-crossing triggers for banner_drop, saloon_doors_swing,
//    turnstile_spin, doors_open/doors_close
...
export class Interactions {
    constructor({ scene, camera, rig, mixer, clips, dom }) {
        this.scene = scene;
        this.camera = camera;
        this.rig = rig;
        this.mixer = mixer;
    ...
    _collect() {
        this.scene.traverse((o) => {
            ...
            if (o.name === 'WP3_Batwing_L') this.batwing = o;
            if (o.name === 'WP4_Turnstile_Rotor') this.turnstile = o;
        });
        // swaying mine lanterns (+ a warm light in each)
        this.lanterns = [];
        this.scene.traverse((o) => {
            if (/^WP4_Lantern_\d+$/.test(o.name)) {
                ...
                li.position.copy(body ? body.position : new THREE.Vector3(0, 0, -0.38));
                o.add(li);
                this.lanterns.push({
                    pivot: o,
                    baseX: o.rotation.x,
    ...
    _paintBanner() {
        ...
        img.src = encodeURI('./images/Dennis-Münch-Portrait.jpg');
    }
    ...
    _setupTriggers() {
        ...
        this.triggers = [];
        const add = (u, fn, once) => this.triggers.push({ u: ((u % 1) + 1) % 1, fn, once, done: false });
        if (this.batwing) {
            const p = new THREE.Vector3();
            this.batwing.getWorldPosition(p);
            add(this.rig.uOfPoint(p) - 0.004, () => this._play('saloon_doors_swing'), false);
        }
        if (this.turnstile) {
            const p = new THREE.Vector3();
            this.turnstile.getWorldPosition(p);
            add(this.rig.uOfPoint(p) - 0.003, () => this._play('turnstile_spin'), false);
        }
        add(a.contact - 0.008, () => this._play('doors_open'), false);
        add(a.contact_inside + 0.02, () => this._play('doors_close'), false);
    }
    ...
    update(dt) {
        ...
        // crossing detection (direction-agnostic, wrap-aware)
        for (const tr of this.triggers) {
            if (tr.once && tr.done) continue;
            if (crossed(this.prevU, u, tr.u)) {
                tr.fn();
                tr.done = true;
            }
        }
        ...
        // banner: unrolled while approaching through the plaza/atrium,
        // rolled up again just before the camera reaches the archway
        if (this.banner) {
            const ahead = ((this.uBanner - u) % 1 + 1) % 1;
            const wantDropped = this.rig._moved && ahead > 0.018 && ahead < 0.24;
            const target = wantDropped ? 0.0 : 1.0;
            const speed = dt * (target > this.bannerInf ? 1.1 : 0.55);
            this.bannerInf += Math.max(-speed, Math.min(speed, target - this.bannerInf));
            this.banner.morphTargetInfluences[this.bannerMorph] = this.bannerInf;
        }

        // menu book: flip spreads while scrolling at the bar
        const aboutStop = this.rig.stops.find((st) => st.name === 'about');
        if (aboutStop && this.bookSpreads) {
            const s = ((this.rig.current % 1) + 1) % 1;
            const R = 0.045;
            const p = (s - (aboutStop.s - R)) / (2 * R);
            if (p > -0.2 && p < 1.2) {
                this._setSpread(Math.max(0, Math.min(1, Math.floor(p * 2))));
            }
        }

        for (const ln of this.lanterns) {
            ln.pivot.rotation.x = ln.baseX + Math.sin(t * 1.15 + ln.phase) * 0.16;
            ln.pivot.rotation.z = ln.baseZ + Math.cos(t * 0.9 + ln.phase * 1.3) * 0.11;
        }
